...
clipboard_service="${XDG_CONFIG_HOME:-$HOME/.config}/systemd/user/ayame-clipboard.service"
...
systemctl --user disable --now ayame-clipboard.service >/dev/null 2>&1 || true
rm -f "$shell_service" "$clipboard_service"
